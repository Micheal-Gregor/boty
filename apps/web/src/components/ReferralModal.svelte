...
{#if r}
  <div class="ent-overlay">
    <div class="confirm" onclick={(e) => e.stopPropagation()}>
      ...
      <p class="cbody"><strong>{referrer}</strong> can't do this one — it's your trade. Take <strong>{r.job?.name ?? "the job"}</strong> ({$money(r.job?.value)}) and you'll owe them a <strong>{$money(r.fee)}</strong> finder's fee. Or pass and it walks.</p>
      <div class="cbtns">
